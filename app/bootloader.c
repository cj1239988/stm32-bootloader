...
static void bl_rx_handler(const uint8_t *data, uint32_t size)
{
    ...
    {
        //printf("recv: %02X\n", *data);
        packet_buffer[packet_index++] = *data++;// 将接收到的数据存入缓冲区
        switch(packet_state)
        {
...
void bootloader_main(void)
{
    ...
    bl_usart_init();
    bl_usart_register_rx_callback(bl_rx_handler); // 注册接收回调函数
    while(1)
    {
        if(packet_received)
        {
